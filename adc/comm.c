...
void UART_init(void)
{
    ...
    uart_CTL1_value = BIT6 | BIT0; //bit6 set and bit7 reset = ACLK
    UCA0CTL1 = uart_CTL1_value;

    //baud rate configuration (check with xbee ->1200bps)
    /* N = f(BRCLK)/Baud rate, f(BRCLK) = 32.768KHz, baud rate is 1200bps. N = 27.3066
     * low freq. mode, UCBRx = INT(N) = 27, UCBRSx = 2, UCOS16 = 0
     */
    UCA0BR0 = 27;
    UCA0BR1 = 0;
    UCA0MCTL = 0x2 << 1;
